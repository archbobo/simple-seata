...
import io.spring2go.seata.core.protocol.*;
import io.spring2go.seata.core.rpc.ClientMessageListener;
import io.spring2go.seata.core.rpc.ClientMessageSender;
import io.spring2go.seata.core.rpc.RemotingService;
import io.spring2go.seata.core.service.ServiceManager;
import io.spring2go.seata.core.service.ServiceManagerStaticConfigImpl;
...
public abstract class AbstractRpcRemotingClient extends AbstractRpcRemoting implements RemotingService, RegisterMsgListener, ClientMessageSender {
    ...
    protected Channel getNewChannel(InetSocketAddress address) {
        ...
        try {
            ...
            if (f.isCancelled()) {
                throw new FrameworkException("connect concelled, can not connect to seata-server.");
            } else if (!f.isSuccess()) {
                throw new FrameworkException("connect failed, can not connect to seata-server.");
            } else {
                channel = f.channel();
            }
        } catch (Exception e) {
            throw new FrameworkException(e, "can not connect to seata-server.");
        }
        return channel;
    }
    ...
    public void channelRead(final ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (((RpcMessage)msg).getBody() instanceof MergeResultMessage) {
            ...
            MergedWrapMessage mergeMessage = (MergedWrapMessage)mergeMsgMap.remove(((RpcMessage)msg).getId());
